...
import { HOURS_ONLY } from "../constants/timeModes";
  ...
  return (
    <>
      <article
        ...
        justify-between h-full p-5 bg-gray-600
        rounded-sm"
      >
      ...
      <article className="p-5 bg-gray-600 rounded-sm">
        <h1 className="text-center text-3xl mb-3">Hourly</h1>
        <article className="flex flex-row">
          {props.searchHourly.map((hour, index) => {
              ...
              return (
                ...
                >
                  <p className="text-xs text-gray-200 sm:text-lg">
                    {convertTime(hour.dt, HOURS_ONLY)}
                  </p>
                  <img
                    className="cardImage"
            ...
            } else return <Fragment key={hour.dt}></Fragment>;
          })}
        </article>
      </article>
    </>
...
export default connect(mapState)(
  withWeatherContainer(CurrentWeather)
);
